...
class DB:
    ...
    def add_city(self, region_id: int, name: str) -> schemas.CityID:
        res = self.cur.execute("""INSERT INTO cities (region_id, name) 
                               VALUES (?, ?)""", (region_id, name))
        self.con.commit()
        res = self.cur.execute("""SELECT city_id FROM cities ORDER BY
                               city_id DESC LIMIT 1""")
        return schemas.CityID(city_id=res.fetchone()[0])

    def get_all_cities(self) -> list[schemas.City]:
        res = self.cur.execute("""SELECT city_id, region_id, name 
                               FROM cities ORDER BY city_id DESC""")
        answer = list()
        for row in res.fetchall():
            region_res = self.cur.execute("""SELECT name FROM regions
    ...
    def add_place(self, title: str, city_id: int, youtube_link: str,
                  content: str, latitude: float,
                  longitude: float) -> schemas.PlaceID:
        youtube_id = re.search(YOUTUBE_ID_PATTERN, youtube_link)[0]
        res = self.cur.execute(
            """INSERT INTO places (title, city_id, youtube_id, 
            content, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?)""",
            (title, city_id, youtube_id, content, latitude, longitude))
        self.con.commit()
        res = self.cur.execute(
            "SELECT place_id FROM places ORDER BY place_id DESC LIMIT 1")
    ...
    def get_places(self, count: int, offset: int) -> list[schemas.Place]:
        res = self.cur.execute(
            ...
            ORDER BY place_id DESC LIMIT ? OFFSET ?""", (count, offset))
        answer = list()
        for row in res.fetchall():
    ...
    def get_places_by_city_id(self, city_id: int, count: int, offset: int) -> schemas.Place:
        res = self.cur.execute(
            """SELECT place_id, title, city_id, youtube_id,
            content, latitude, longitude FROM places WHERE city_id=?
            ORDER BY place_id DESC LIMIT ? OFFSET ?""", (city_id, count, offset))
    ...
    def get_places_by_region_id(self, region_id: int, count: int, offset: int) -> schemas.Place:
        ...
        for city in cities:
            res = self.cur.execute(
                ...
                city_id=? ORDER BY place_id DESC LIMIT ?
    ...
    def add_user(self, login: str, password: str) -> schemas.UserID:
        res = self.cur.execute("INSERT INTO user (login, password) VALUES (?, ?)",
                               (login, password))
        self.con.commit()
    ...
    def place_and_images(self, place_id: int, image_id: int) -> None:
        res = self.cur.execute("""INSERT INTO places_and_images 
                               (place_id, image_id) VALUES (?, ?)""",
                               (place_id, image_id))
        return
